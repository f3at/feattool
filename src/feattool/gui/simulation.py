...
from feattool.gui import command, errors, menu, agentinfo, agentview
from feattool.gui.dlg import help_agent
from feattool.core import settings


class Controller(log.Logger):
    """
    ...
    def __init__(self, model):
        ...
        self.builder.add_from_file(data.path('ui', 'simulation.ui'))

        self.info('Loading main simulation window...')
        self.view = MainWindow(self, self.builder, self.model.driver)
        self.view.window.show_all()
        self.info('Done loading main simulation window...')
...
class MainWindow(object):
    """
    This is the view of main simulation window.
    """
    ...
    def reset_toolbar_clicked(self, button):
        dlg = gtk.MessageDialog(
            self.window, gtk.DIALOG_DESTROY_WITH_PARENT, gtk.MESSAGE_QUESTION,
            gtk.BUTTONS_YES_NO,
            gettext('All agents will be terminated. Do you want continue?'))
